...
We will need to define some types for the Movie data we will be using in our React app. In VS Code, in the `src` folder, add a file `types.ts` :
...
const MovieCard: React.FC<{movie: BaseMovie}> = (props) => {
...
Create the file `src/stories/filterMoviesCard.stories.tsx` and add the following:
