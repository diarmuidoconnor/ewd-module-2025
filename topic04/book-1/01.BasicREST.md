...
$ cdk deploy
~~~
The above code's effect is to add a REST web API with two resources, both configured with a Lambda Proxy integration. The two endpoints are:
...
export const handler: APIGatewayProxyHandlerV2 = async (event, context) => { // Note change
  try {
    ...
    if (!movieId) {
      ... as before ...
~~~
A cosmetic change is also recommended to `lambdas/getAllMovies.ts`. Change the first import and the handler's signature as shown below:
~~~ts
