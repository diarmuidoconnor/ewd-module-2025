...
import { DiscoverMovieOverviewProps } from "../types/movieApp";// Changed
...
![image-20260325162046272](./img/image-20260325162046272.png)
...
import { MovieDetailsProps , MovieImage} from "../types/movieApp";// replace existing MoviePageProps import
...
Create a new type to restrict the Filter options to either `"title"` or `"genre"`. In `src/types/movieApp.ts`, add the following type:
...
In `src/components/FilterMoviesCard.tsx`, make the changes shown below:
~~~tsx
import { useState, useEffect, ChangeEvent } from "react";//update existing import
...
const FilterMoviesCard = ({ titleFilter, genreFilter }: FilterMoviesCardProps) => {
