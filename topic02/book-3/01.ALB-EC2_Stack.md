...
$ npm install
~~~
In `bin/cdk-starter.ts`, add the environment for the stack as shown below, using your 12-digit AWS account identification number:
~~~ts
new CdkStack(app, 'cdk-stack', {
  stackName: 'cdk-stack', 
    env: { account: '517123456789', region: 'eu-west-1' },    // NEW
});
~~~
...
__NOTE:__ If you get the above response, then the deployment was successful, and you can continue completing this section. However, occasionally the load balancer returns a __504 Bad Gateway__ error because it believes the EC2 instances are unhealthy. As we will not be relying on this type of infrastructure (EC2, Load Balancers, etc), it is not necessary to debug the problem. Instead, you should skip to the next page, where we deploy a simpler stack. 

The above response confirms the web server inside the EC2 instance is running. The web page displays a Hello World message followed by the server's hostname, which includes the private IP address of the EC2 instance. Hit refresh a few times and notice the hostname alternates between two IP addresses - see `lib/user-data.sh`. Our stack provisioned two EC@ instances, and the Application Load Balancer distributed the requests between the two targets.
...
Go to the final section of this lab: the Cleanup.
