...
##### If you get the above response, then the deployment was successful. However, occasionally the load balancer returns a __504 Bad Gateway__ error because it believes the EC2 instances are unhealthy. As we will not be relying on this type of infrastructure (EC2, Load Balancers, etc), it is not necessary to debug the problem. Instead, you should skip to the next page, where we deploy a simpler stack. If you did not get an error, then continue with the remainder of this section.
