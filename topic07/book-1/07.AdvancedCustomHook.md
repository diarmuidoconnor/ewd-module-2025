...
We will take this opportunity to refactor the filtering UI code before using the hook. The objective of the refactoring is to allow for other types of filtering in the future. Create the file `components/MovieFilterUI.tsx` and add this code:
~~~tsx
...
~~~
In `components/TemplateMovieListPage.tsx`, replace the entire content with the following - we are removing the filtering UI code from the template:
~~~tsx
