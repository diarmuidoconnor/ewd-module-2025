...
+ GET /movies/{movieID}/reviews[?reviewer=reviewerID] - Get all the reviews for the specified movie. 
     + Allow an optional query string parameter that specifies a reviewer ID (username), e.g. ?reviewer=userA. Only the movie review by that person should now be returned.
