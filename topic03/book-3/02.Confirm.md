...
In `lib/auth-app-stack.ts`, add a second resource to the Auth API using the private function as follows:
~~~ts
    // Unchanged
    this.addAuthRoute(
      "signup",
...
~~~
The execution flow in the above handler function is similar to the sign-up function. Use Postman to test it, where the body of the POST request is the following:
~~~ts
{
    "username": "userA",
...
In the AWS management console, go to Cognito --> User pools (left panel) --> Click your user pool, and notice the Confirmed status of userA:
