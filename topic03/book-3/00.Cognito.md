...
In this lab, we will use the AWS Cognito service to add user authentication to a simple web API. We will provision the following AWS resources to demonstrate this:

+ A Cognito User pool.
+ An API to perform user authentication - The Auth web API.
+ A REST API to demonstrate public and protected endpoints - the App API.
+ A custom authorizer lambda function to prevent unauthorized requests accessing protected routes.

A visual representation of the CDK stack is demonstrated below:

![][arch]

The Auth API will provide endpoints that allow a user to issue the following requests:
...
The authentication API (Auth API) will have several L2 constructs similar in structure. By creating a private method for declaring these common elements, we reduce code duplication. In `lib/auth-app-stack.ts`, define the method as follows:
~~~ts
