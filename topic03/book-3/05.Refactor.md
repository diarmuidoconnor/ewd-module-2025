...
The CDK framework provides two main concepts for structuring our infrastructure code: Stacks and Construct. The framework defines lots of L2 Constructs, which are synthesised into Cloudformation resource declarations, and used to provision an app's infrastructure, e.g. API Gateway resources, Lambda instances, Cognito user pool. 

To date we have placed all infrastructure code inside one Stack instance, but multi-stack CDK apps are possible. Also, we can extract a logical code block in a stack, and encapsulate it inside a custom L2 Construct. To demonstrate, we will refactor the code developed in this lab as follows:

![][construct]

Before refactoring our code, remove the deployed stack from your AWS account:
~~~bash
$ cdk destroy
~~~

Create the folder `lib/constructs`, and add the file `lib/constructs/auth-api.ts` with the following code:
~~~ts
...
~~~
The above code is identical to what we developed in the Stack class (`lib/auth-app-stack.ts`) - we will remove it from there shortly. Create the file `lib/constructs/app-api.ts` and add the following code:
~~~ts
