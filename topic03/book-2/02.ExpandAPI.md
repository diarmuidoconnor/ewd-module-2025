...
'isValidMoviePayload' is a dynamically generated function to validate a JS object against the Movie schema produced by typescript-json-schema. Using cmd-k/cmd-i to check the type of the body variable before anf after the if-statement. It changes from the 'any' type to Movie, i.e. the isValidMoviePayload has performed __type narrowing__. 
...
###  Typescript Type Guards 
 ...
 Instead of using the AJV library, we could have written our own validator, using Typescript's [type guard][guard] feature.  Add the following function to the bottom of 'addMovie.ts':
~~~ts
...
~~~
[For convenience, we are assuming a movie has only four properties: id, title, overview, and genre_ids.] The non-standard signature of the function makes it a type guard. In the lambda handler, replace the statement:
~~~
    if (!isValidMoviePayload(body) ) {
~~~
with
~~~
    if (!isMovie(body) ) {
~~~
The type of the body variables narrows from the any type to Movie, as before. Redeploy the stack and test it as before.
