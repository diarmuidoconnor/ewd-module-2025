...
```
The render prop's function returns the icon to display in movie cards on the home page. We must pass the render prop down the component hierarchy to the movie card component, which ultimately will invoke it. In `types/moviesAppTypes.ts`:
...
In `components/TemplateMovieListPage.tsx`:
...
```
+ Similarly, in `components/MovieList.tsx`, modify the  destructured properties and the return statement's `Movie` component use to the following:
...
```
In `components/MovieCard.tsx`, update the component code as follows:
...
+ Replace the lines 
  ...
  ~~~tsx
  const { favourites, addToFavou } = useContext(MoviesContext);
  ~~~
