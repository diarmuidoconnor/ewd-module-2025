...
~~~
We will also add a component to display a spinner when the app is waiting for data from TMDB. Create the file `src/components/Spinner.tsx`:
~~~tsx
...
~~~
Before using React-Query in our components, we will define types for the movies and genres data that describe the "shape" of the data returned from the API .  In ``src/types/movieAppTypes.ts`` , add the following two interface that describes the list of movies and genre data returned by the TMDB API:
