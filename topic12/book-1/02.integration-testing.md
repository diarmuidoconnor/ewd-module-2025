...
+ To test the request, make sure to start the API developed in the previous labs. Then click on **Send** in the postman request. You should see the following result:
...
+ Now hit the *Run * button again. This time you should see a result similar to the following:
...
By Using [ChaiJS BDD](https://www.chaijs.com/api/bdd/) syntax and `pm.expect` , we can also check details of the response. For example, creating an account should return a UUID for the account (remember this is gerenrated by the Database).
...
+ Run the test again. You should see the results for 2 tests:
...
+ Add the following tests to check for a correct response: 
