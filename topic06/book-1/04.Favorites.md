...
~~~
In `src/types/movieAppTypes.ts`, update the  `DiscoverMovieOverviewProps` and `MovieDetailsProps` types to include a favourite property.  Replace both with the following:
...
Also, in the same file, add/change the following:
...
In `components/MovieList.tsx`, replace the following lines to use the new interface description
...
~~~
This will cause compile errors for now - we will fix those next....

In `components/MovieCard.tsx`, add/change the following change to accommodate the new event prop:
