...
In `src/components/MovieCard.tsx` make two changes:
...
Create a new type to restrict the Filter options to either `"title"` or `"genre"`. In `src/types/movieAppTypes.ts`, add the following type:
