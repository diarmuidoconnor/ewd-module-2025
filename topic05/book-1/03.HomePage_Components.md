...
Go to the following URL(https://developer.themoviedb.org/openapi/tmdb-api.json) and save into /src/api as `tmdb-api.json`.
...
`NonNullable` is there to remove any `null` or `undefined` from the generated API type before you use it.

Some  OpenAPI fields are likely optional, So this will  strip out null and undefined and treat it as a definite array.

This is the **what you’ll use in your app** to describe the shape of interfaces/types. We will add to this as we go along...
...
In VS Code, create the folder `src/components/MovieCard.tsx` :
...
import { DiscoverMovieOverviewProps } from "../types/movieApp";
