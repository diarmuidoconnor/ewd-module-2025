...
In this lab, we commence the development of a Single Page App(SPA) for Movie enthusiasts. The product will go through several iterations over the coming weeks, adding new functionality and refactoring existing code during that period. The app's data source will be the open web API [The Movie Database](https://developers.themoviedb.org/3/getting-started/introduction ).  Your API includes an [OpenAPI specification](https://developer.themoviedb.org/openapi) that will serve as the single source of truth for all data types used in the application.

This approach makes sense because the application is largely driven by the API, with most of its functionality relying on it. By using the OpenAPI definition as the authoritative source, you ensure:

- Consistency between frontend and backend
- Strong typing across the application
- Easier code generation (e.g., clients, models, validation)
- Reduced duplication and fewer integration errors
