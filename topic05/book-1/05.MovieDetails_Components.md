...
In VS Code, create the file `src/components/MovieDetails.tsx` and add the following:
...
import { MovieDetailsProps } from "../types/movieApp";
...
~~~
Using the map method in this way is a common pattern in React development. The ?? is there to check that genre is present as a property. 

### The MovieHeader component.

Create the file `src/components/HeaderMovie.tsx` and add the following code:
~~~tsx
